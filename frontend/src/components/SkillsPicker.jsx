...
const SkillRow = ({ skill, checked, onToggle }) => {
  return (
    <label className="group flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-800 bg-slate-950/80 px-3 py-2 transition hover:border-brand-400 hover:bg-slate-900">
      <input type="checkbox" checked={checked} onChange={() => onToggle(skill.id)} className="h-4 w-4 accent-brand-500" />
  ...
  return (
    <div className="panel">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">🧠 Skills & Technologies</h2>
          ...
          <div className="flex flex-wrap gap-2">
            {['shields', 'icons'].map((style) => (
              <button
                ...
                className={`rounded-2xl px-4 py-2 text-sm font-medium transition ${skillStyle === style ? 'bg-gradient-to-r from-brand-500 to-fuchsia-500 text-white shadow-glow' : 'bg-slate-950 text-slate-300 hover:bg-slate-900'}`}
              >
      ...
      <div className="mt-6 space-y-5">
        {filteredCategories.map((category) => (
          <div key={category.category} className="panel-inset p-4">
            <button
              type="button"
